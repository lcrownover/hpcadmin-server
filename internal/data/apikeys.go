...
func GetAPIKeyEntry(db *sql.DB, key string) (*APIKeyEntry, error) {
	slog.Debug("querying database for api key", "method", "GetAPIKeyEntry")
	var k APIKeyEntry
	...
	if err != nil {
		if err == sql.ErrNoRows {
			slog.Debug("api key not found in database", "method", "GetAPIKeyEntry")
			return nil, fmt.Errorf("no APIKeyToken found with provided key")
		}
		slog.Debug("failed to look up key from database", "method", "GetAPIKeyEntry", "error", err)
		return nil, err
	}
	slog.Debug("found api key in database", "method", "GetAPIKeyEntry")
	return &k, err
}
